...
import {deleteItemRequest, getAllServicesRequest} from "../slices/listSlice";
...
function List() {
  ...
  const { items, loading, error, itemsOnDelete } = useSelector((store) => store.listSlice);
  ...
  if (itemsView.length === 0) {
    return <p className="list-empty">Список услуг пуст.</p>;
  }
